...
public sealed class FocusClassificationClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var prompt =
            ...
            "不要替用户安排明天的计划，不要输出命令式 next action。suggestedNextAction 字段请写成一个自然、亲和的问题，引导用户自己决定下一步，例如询问明天开场最想先接住哪件事。\n" +
            "结构要清楚，但语气要像熟悉的伙伴在桌边复盘：温和、具体、有支撑感。encouragement 字段请写 1-2 句鼓励/安慰：承认今天真实发生过的努力，不夸张，不空泛，不否定疲惫。\n\n" +
            $"当前专注目标：{NormalizeFocusGoal(_settingsService.Settings.FocusGoal)}\n" +
            "今日任务：\n" + string.Join('\n', taskLines) + "\n\n" +
            "本地记忆星图：\n" + string.Join('\n', memoryLines) + "\n\n" +
            "最近24小时进程切换聚合：\n" + string.Join('\n', appLines) + "\n\n" +
            "必须只返回 JSON，不要 Markdown，不要解释。格式：\n" +
            "{\"encouragement\":\"今天不只是窗口在切换，你确实把注意力落到了一些具体的事情上。就算节奏有点断，也已经留下了可以继续接住的线索。\",\"review\":\"今天主要在写作和代码验证之间来回推进，比较像是在把想法和实现慢慢对齐。\",\"highlights\":[\"论文摘要有推进\",\"WPF 桌宠和统计界面继续变清晰\"],\"risks\":[\"有些任务可能还没有被拆到足够轻\"],\"suggestedNextAction\":\"如果明天只选一个开场动作，你更想先接住哪件事？\"}";
